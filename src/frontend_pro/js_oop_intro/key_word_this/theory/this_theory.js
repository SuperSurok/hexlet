...
{
    // Для доступа к данным объекта внутри метода используется ключевое слово constructor
    // constructor внутри функции ссылается на ближайший внешний объект или на текущий объект
    // к которму привязан метод
    // Так constructor работает для обычных функций
...
{
    // В JS constructor ссылается на текущий объект к которму привязан метод
    // constructor может измениться прямо во время исполнения программы
    const company1 = {
        name: 'Hexlet',
    ...
    // Или, другими словами в момент обращения к constructor
}

{
    // Про метод вызова функции call
    // Первым параметром передаётся контекст, объект на который
    // будет ссылаться constructor внутри функции
